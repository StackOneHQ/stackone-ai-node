...
StackOne provides two powerful toolsets:
...
// 1️⃣ Method 1: Set at toolset initialization
const toolset = new StackOneToolSet({ accountId: "your-account-id" });

// 2️⃣ Method 2: Set when getting tools (overrides toolset account ID)
const tools = toolset.getTools("hris_*", "override-account-id");

// 3️⃣ Method 3: Set directly on a tool instance
...
> 💡 **Note**: you can build your own custom transformations using both toolset classes. See the [Parameter Transformations](#-advanced-parameter-transformations) section for more information.
...
The OpenAPIToolSet supports easy usage of both Basic and Bearer authentication:
...
// 1️⃣ Schema Override: Simplify the input schema
...
// 2️⃣ PreExecute: Transform the simplified parameters back to API format
...
Made with ❤️ by [StackOne](https://stackone.com)
